...
const P = (() => {
  ...
  function _uploadLocal(toupload, existingItems, playlist, options) {
    ...
    //bleubird reads wierd
    return Q.map(toupload, (vo) => {

      return new Q((resolve, reject) => {

        if (!fs.existsSync(vo)) {
          console.log(colors.red(`Dont exist ${vo}`));
          return resolve()
        }
        ...
        console.log(colors.green(`Uploading ${vo} with title: ${title} at index ${toupload.indexOf(vo)}`));
  ...
  function _dispatchEvent(str, val) {
    if (events[str].length) {
      events[str].forEach(func=>{
        func(val)
      })
    }
  }
